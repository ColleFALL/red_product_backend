...
#             traceback.print_exc()
#             return Response({"success": False, "message": "Erreur serveur", "detail": str(e)}, status=500)
...
def ok(message: str = "", data=None, code: int = 200) -> Response:
    """Réponse succès standardisée"""
    return Response({"success": True, "message": message, "data": data}, status=code)


def fail(message: str = "", data=None, code: int = 400) -> Response:
...
class LoginView(APIView):
    ...
    def post(self, request):
        try:
            ...
            traceback.print_exc()
            return Response(
                {"success": False, "message": "Erreur serveur", "detail": str(e)},
                status=500
            )
...
class ForgotPasswordView(APIView):
    ...
    def post(self, request):
        ...
        if not email:
            # Toujours retour générique pour sécurité
            return ok("Si cet email existe, vous recevrez un message.", None, 200)

        user = Admin.objects.filter(email=email).first()
        if user:
            prt = PasswordResetToken.create_for(user, hours=1)
            # DEBUG : on log le token
            print(f"[FORGOT PASSWORD] email={email} token={prt.token} expires={prt.expires_at}")
